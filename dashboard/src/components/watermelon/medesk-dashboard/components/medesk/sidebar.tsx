...
function NavItem({ item }: { item: NavigationItem }) {
  ...
  return (
    <SidebarMenuButton tooltip={item.name} className={menuButtonClassName} render={<DashboardLink href={item.href} aria-current={isActive ? "page" : undefined} onClick={() => {
                if (isMobile) setOpenMobile(false);
              }} />}><item.icon /><span>{item.name}</span>{item.badge ? <SidebarMenuBadge>{item.badge}</SidebarMenuBadge> : null}</SidebarMenuButton>
  );
}
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent side="top" align="start" className="w-56">
                  ...
                  <DropdownMenuGroup>
                    ...
                    <DropdownMenuItem render={<a href="https://docs.medesk.com" target="_blank" rel="noreferrer" />}><BookOpenIcon className="size-4" /><span>Documentation</span></DropdownMenuItem>
                    <DropdownMenuItem render={<a href="https://medesk.com/support" target="_blank" rel="noreferrer" />}><MessageCircleIcon className="size-4" /><span>Contact support</span></DropdownMenuItem>
                  </DropdownMenuGroup>
                  <DropdownMenuSeparator />
